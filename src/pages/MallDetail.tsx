...
import { Building2, MapPin, Search, Filter, X, Heart, Navigation, Clock, Phone, Globe } from 'lucide-react';
import { getMall, getMallByName, listStores, listFloors } from '../services/firebase/firestore';
...
export default function MallDetail() {
  ...
  useEffect(() => {
    ...
    console.log('🔍 Loading mall data for:', mallId);
    ...
    const loadMallData = async () => {
      try {
        ...
        // Load mall details by name (slug)
        console.log('📡 Fetching mall by name:', mallId);
        const mallData = await getMallByName(mallId);
        if (!mallData) {
        ...
        // Load floors
        console.log('📡 Fetching floors for mall:', mallData.id);
        const floorsData = await listFloors(mallData.id!);
        console.log('✅ Floors loaded:', floorsData);
        setFloors(floorsData);
        
        // Load stores
        console.log('📡 Fetching stores for mall:', mallData.id);
        const storesData = await listStores(mallData.id!);
        console.log('✅ Stores loaded:', storesData);
        setStores(storesData);
        
      } catch (err) {
        console.error('❌ Error loading mall data:', err);
  ...
  const filteredStores = useMemo(() => {
    ...
    return stores.filter(store => {
      // Category filter
      if (!filters.categories.includes('all') && !filters.categories.includes(store.category || '')) {
        return false;
      }
      
      // Floor filter
      if (!filters.floors.includes('all') && !filters.floors.includes(store.floorId || '')) {
        return false;
      }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">ไม่พบข้อมูลห้าง</h2>
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button 
                    ...
                    onClick={() => setFilters(prev => ({ ...prev, floors: ['all'] }))}
                  >
                    ทั้งหมด
                  </button>
                  {floors.map(floor => (
                    <button
                      key={floor.id}
                      className={`px-3 py-2 rounded-lg text-sm font-medium transition-all ${
                        filters.floors.includes(floor.id) 
                          ? 'bg-green-600 text-white' 
                          : 'bg-gray-100 text-gray-600 hover:bg-green-50 hover:text-green-600'
                      }`}
                      onClick={() => setFilters(prev => ({ ...prev, floors: [floor.id] }))}
                    >
                      {floor.label}
                    </button>
                  ))}
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-gray-600">พบ <span className="font-medium">{filteredStores.length}</span> ร้าน</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
              {filteredStores.map(store => (
                <StoreCard key={store.id} store={store} floorLabel={getFloorLabel(store.floorId || '')} />
              ))}
            </div>
            ...
            {filteredStores.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">ลองปรับตัวกรองหรือคำค้นหาใหม่</p>
                <button 
                  onClick={clearFilters}
                  className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
                >
...
function StoreCard({ store, floorLabel }: { store: Store; floorLabel: string }) {
  const getCategoryColor = (category: string) => {
   ...
   return (
     <div className="bg-white rounded-2xl shadow-sm border hover:shadow-lg hover:border-green-200 transition-all duration-200 overflow-hidden">
       <div className={`aspect-video bg-gradient-to-br ${getCategoryColor(store.category || '')} flex items-center justify-center`}>
         <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center">
           <span className="text-2xl font-bold text-gray-700">{getInitials(store.name)}</span>
         </div>
       </div>
       <div className="p-6">
         <div className="flex items-start justify-between mb-3">
           <div className="flex-1">
             ...
             <div className="flex items-center space-x-2 mb-3">
               ...
               {store.phone && (
                 <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-lg font-medium">Phone</span>
               )}
               {store.hours && (
                 <span className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded-lg font-medium">Hours</span>
               )}
             </div>
           </div>
